...
class AlgorithmService:
    ...
    def generate_aoi_cache(
        ...
        Thumbnails are written as loose files during detection (safe for
        concurrent/multi-process access). They are migrated to a ZIP archive
        at viewing time for fast file copy to/from USB drives.
        ...
        try:
            ...
            # Set up thumbnail cache directory (needed as fallback for loose files)
            thumbnail_cache_dir = Path(output_dir) / '.thumbnails'
            thumbnail_cache_dir.mkdir(parents=True, exist_ok=True)

            # Initialize cache services
            # NOTE: Do NOT use ZIP store during detection -- multiple processes may
            # write concurrently. Write loose files here; migration to ZIP happens
            # at viewing time in ThumbnailCacheService._init_zip_store().
            thumbnail_service = ThumbnailCacheService(dataset_cache_dir=str(thumbnail_cache_dir))
            thumbnail_service.zipStore = None  # Force loose-file writes during detection
            ...
            for aoi in areas_of_interest:
                try:
                    ...
                    # Save thumbnail as loose file (concurrent-safe)
